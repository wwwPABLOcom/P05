...
def extract_features_song(ruta_cancion):
    try:
        ...
        mfccs_var = np.var(mfccs, axis=1) # NUEVO: Varianza
        
        # 2. Chroma
        chroma = librosa.feature.chroma_stft(y=audio_data, sr=sampling_rate)
        chroma_mean = np.mean(chroma, axis=1)
        chroma_var = np.var(chroma, axis=1) # NUEVO: Varianza
        
        # 3. Spectral Centroid
        centroid = librosa.feature.spectral_centroid(y=audio_data, sr=sampling_rate)
        centroid_mean = np.mean(centroid, axis=1)
        centroid_var = np.var(centroid, axis=1) # NUEVO: Varianza
        
        # 4. Zero Crossing Rate
        zcr = librosa.feature.zero_crossing_rate(y=audio_data)
        zcr_mean = np.mean(zcr, axis=1)
        zcr_var = np.var(zcr, axis=1) # NUEVO: Varianza
        
        # Apilamos TODO (medias y varianzas)
...
def generate_features_and_labels(ruta_datos='../P05/Data/genres_original/'):
...
caracteristicas_finales, etiquetas_one_hot, etiquetas_unicas = generate_features_and_labels('../P05/Data/genres_original/')
...
# ... (Todo tu código anterior hasta el print de accuracy) ...
print(f"\nPrecisión en el conjunto de test: {accuracy:.4f}")

######################################################### 
# PARTE 4: Guardar el modelo y los transformadores para uso futuro
######################################################### 
print("\nGuardando el modelo y los datos de preprocesamiento...")

# 1. Guardar la Red Neuronal entrenada
model.save('mi_modelo_musical.h5')

# 2. Guardar el Scaler (Crucial: la IA necesita escalar las canciones nuevas exactamente igual)
joblib.dump(scaler, 'mi_escalador.pkl')

# 3. Guardar el nombre de las etiquetas (para saber qué número es Rock, Jazz, etc.)
np.save('mis_etiquetas.npy', etiquetas_unicas)

print("¡Todo guardado correctamente! Ya no necesitas volver a entrenar.")