...
MODELO_PATH   = "modelo_hibrido.keras"
SCALER_PATH   = "scaler_hibrido.pkl"
CLASES_PATH   = "clases_hibrido.json"

IMG_SIZE      = (128, 128)    # Más pequeño que antes → más rápido, suficiente
BATCH_SIZE    = 16
EPOCHS_FASE1  = 25
EPOCHS_FASE2  = 15
...
def audio_a_espectrograma(y: np.ndarray, sr: int) -> np.ndarray:
    """
    Convierte fragmento de audio en espectrograma Mel como imagen RGB.
    Devuelve array (IMG_SIZE[0], IMG_SIZE[1], 3) normalizado [0,1].
    """
...
def construir_modelo_hibrido():
    """
    Modelo de 3 ramas que se fusionan:

    Rama A (CSV features):
        Input(57) → Dense(128,relu) → Dense(64,relu)

    Rama B (Imagen PNG):
        Input(128,128,3) → MobileNetV2 → GlobalAvgPool → Dense(128,relu)

    Rama C (WAV features):
        Input(57) → Dense(128,relu) → Dense(64,relu)

    Fusión:
        Concatenate([A, B, C]) → Dense(256,relu) → Dropout(0.4)
    ...
    print("\n" + "=" * 60)
    print("  CONSTRUYENDO MODELO HÍBRIDO DE 3 RAMAS")
    print("=" * 60)

    # ── Rama A: CSV features ──────────────────────────────────────
    inp_csv = layers.Input(shape=(N_FEATURES,), name='input_csv')
    x_csv = layers.Dense(128, activation='relu')(inp_csv)
    x_csv = layers.BatchNormalization()(x_csv)
    x_csv = layers.Dropout(0.3)(x_csv)
    ...
    base_cnn.trainable = False  # Congelada en fase 1
    ...
    # ── Rama C: WAV features (calculadas en tiempo real) ──────────
    inp_wav = layers.Input(shape=(N_FEATURES,), name='input_wav')
    x_wav = layers.Dense(128, activation='relu')(inp_wav)
    x_wav = layers.BatchNormalization()(x_wav)
    x_wav = layers.Dropout(0.3)(x_wav)
    x_wav = layers.Dense(64, activation='relu')(x_wav)
    x_wav = layers.BatchNormalization()(x_wav)
    ...
    modelo = Model(
        inputs=[inp_csv, inp_img, inp_wav],
        outputs=salida,
        name='clasificador_hibrido'
    )
    ...
    print(f"  Rama CSV:  Input(57) → Dense(128) → Dense(64)")
    print(f"  Rama IMG:  MobileNetV2 → GAP → Dense(128)")
    print(f"  Rama WAV:  Input(57) → Dense(128) → Dense(64)")
    print(f"  Fusión:    Concat → Dense(256) → Dense(128) → Softmax(10)")
...
def entrenar(guardar_en: str = MODELO_PATH):
    ...
    print("\n" + "=" * 60)
    print("  FASE 2: Fine-tuning últimas 20 capas de MobileNetV2")
    print("=" * 60)
...
def _grafica_entrenamiento(h1, h2):
    acc    = h1.history['accuracy']    + h2.history['accuracy']
    val    = h1.history['val_accuracy']+ h2.history['val_accuracy']
    corte  = len(h1.history['accuracy'])

    plt.figure(figsize=(10, 5))
    plt.plot(acc, label='Train')
    plt.plot(val, label='Validación')
    plt.axvline(corte - 1, color='gray', linestyle='--', label='Fine-tuning')
    plt.title('Accuracy — Modelo Híbrido')
    plt.xlabel('Época')
    plt.ylabel('Accuracy')
    plt.legend()
    plt.tight_layout()
