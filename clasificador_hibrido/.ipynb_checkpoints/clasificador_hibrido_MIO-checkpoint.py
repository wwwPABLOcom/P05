...
MODELO_PATH   = "modelo_hibrido2(83%).keras"
SCALER_PATH   = "scaler_hibrido2(83%).pkl"
CLASES_PATH   = "clases_hibrido2(83%).json"

IMG_SIZE      = (128, 128)    # Resolución nativa óptima para MobileNetV2   #Antes: (128, 128) # Más pequeño que antes → más rápido, suficiente
BATCH_SIZE    = 16
EPOCHS_FASE1  = 40
EPOCHS_FASE2  = 10
...
def construir_modelo_hibrido():
    ...
    print("\n" + "=" * 60)
    print("  CONSTRUYENDO MODELO HÍBRIDO OPTIMIZADO (Estabilizando >80%)")
    print("=" * 60)

    # ── Rama A: CSV features ──────────────────────────────────────
    inp_csv = layers.Input(shape=(N_FEATURES,), name='input_csv')
    x_csv = layers.GaussianNoise(0.005)(inp_csv)  # Ruido reducido a la mitad
    ...
    x_csv = layers.Dropout(0.3)(x_csv)            # Retiene más información (0.3)
    ...
    x_wav = layers.GaussianNoise(0.005)(inp_wav)  # Ruido reducido a la mitad
    ...
    x_wav = layers.Dropout(0.3)(x_wav)            # Retiene más información (0.3)
...
def entrenar(guardar_en: str = MODELO_PATH):
    ...
    # PACIENCIA AUMENTADA A 20
    callbacks = [
        EarlyStopping(monitor='val_accuracy', patience=20,
                      restore_best_weights=True, verbose=1),
        ReduceLROnPlateau(monitor='val_loss', factor=0.4, patience=5, verbose=1)
    ]
    ...
    h1 = modelo.fit(
        inputs_tr, y_tr,
        epochs=60, # Subido a 60 por si necesita más tiempo
        batch_size=BATCH_SIZE,
        validation_data=(inputs_te, y_te),
        callbacks=callbacks,
        verbose=1
